...
class TestITDlnaLogDetection(unittest.TestCase):
    ...
    def test_status_changed_to_not_scanning_when_log_file_indicates_minidlna_has_finished_scanning(self):
        # Given: the file monitor is started
        self.minidlna_log_monitor = MinidlnaLogMonitor(TEST_FILE)
        self.minidlna_log_monitor.start()
        ...
        # And: the status monitor reports that Pelican is SCANNING
        wait() \
            .with_description("Waiting for status to change") \
            .poll_interval(1, SECOND) \
            .at_most(5, SECOND) \
        ...
        # And: the monitored file logs that miniDLNA is not scanning
        ...
        # And: the status monitor reports that Pelican is NOT_SCANNING
        wait() \
            .with_description("Waiting for status to change") \
            .poll_interval(1, SECOND) \
            .at_most(5, SECOND) \
            .until(lambda: self.status_monitor.last_change_by != "startup")
        self.assertEqual(self.status_monitor.status, Status.NOT_SCANNING)
